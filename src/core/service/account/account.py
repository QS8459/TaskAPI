...
class AccountService(BaseService, JWTHasher):
    ...
    async def auth_user(self, email, password):
        ...
        if instance:
            ...
            if verification:
                access_tkn = self.create_access_token(instance.email, expires_delta= 30)
                refresh_tkn = self.create_refresh_token(instance.email, expires_delta = 1440)
    ...
    async def get_current_user(self, token: str = Depends(request_form)):
        ...
        user: Union[dict[str,Any], None] = await self.get_by_email(email = payload.get('email'))
