...
from src.core.schemas.account import AccountSignInSchema, CustomAuthForm, AccountResponseSchema
...
async def login(
        form_data: CustomAuthForm = Depends(),
        service = Depends(account_service)
):
...
async def me(
        user: AccountSignInSchema = Depends(account_service().get_current_user)
):
    return user
